...
export class InterviewRoundService {
  ...
  findByJobId(jobId: string): Observable<InterviewRound[]> {
    return this.http.get<InterviewRound[]>(`${this.url}?jobId=${jobId}`);
  }
  findByApplicantId(applicantId: string): Observable<InterviewRound[]> {
    return this.http.get<InterviewRound[]>(`${this.url}?applicantId=${applicantId}`);
  }
  findByJobIdAndApplicantId(jobId: string, applicantId: string): Observable<InterviewRound[]> {
